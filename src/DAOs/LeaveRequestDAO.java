...
public class LeaveRequestDAO {
     ...
     * @return 
     */
    public boolean bulkApproveLeaveRequests(List<Integer> requestIds, String approvalNotes, int approverId) {
        ...
        try (Connection conn = getConnection()) {
            ...
            try {
                ...
                for (Integer requestId : requestIds) {
                    LeaveRequestModel request = getLeaveRequestById(requestId);
                    if (request != null && request.isPending()) {
                        request.approve(approvalNotes);
                        if (updateLeaveRequest(request)) {
                            logApprovalAction(conn, requestId, approverId, "BULK_APPROVED", approvalNotes);
                            successCount++;
                        }
                    }
                }
                
                if (successCount == requestIds.size()) {
                    conn.commit();
                    return true;
                } else {
                    conn.rollback();
    ...
    /**
     * Get leave requests requiring urgent approval (within 3 days)
     * @return 
     */
    public List<LeaveRequestModel> getUrgentLeaveRequests() {
        String query = "SELECT leaveRequestId, employeeId, leaveTypeId, leaveStart, leaveEnd, " +
                      "leaveReason, approvalStatus, dateCreated, dateApproved, supervisorNotes " +
                      "FROM leaverequest " +
                      "WHERE approvalStatus = 'Pending' AND leaveStart <= DATE_ADD(CURDATE(), INTERVAL 3 DAY) " +
                      "ORDER BY leaveStart ASC";
        
        List<LeaveRequestModel> urgent = new ArrayList<>();
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    urgent.add(mapResultSetToLeaveRequest(rs));
                }
            }
            
        } catch (SQLException e) {
            System.err.println("Error getting urgent leave requests: " + e.getMessage());
        }
     ...
     * @return 
     */
    public Map<String, Object> getMonthlyLeaveStatistics(int month, int year) {
        String query = "SELECT " +
                      "COUNT(*) as totalRequests, " +
                      "SUM(CASE WHEN approvalStatus = 'Approved' THEN 1 ELSE 0 END) as approvedRequests, " +
                      "SUM(CASE WHEN approvalStatus = 'Rejected' THEN 1 ELSE 0 END) as rejectedRequests, " +
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    statistics.put("totalRequests", rs.getInt("totalRequests"));
                    statistics.put("approvedRequests", rs.getInt("approvedRequests"));
                    statistics.put("rejectedRequests", rs.getInt("rejectedRequests"));
                    statistics.put("pendingRequests", rs.getInt("pendingRequests"));
                    statistics.put("uniqueEmployees", rs.getInt("uniqueEmployees"));
                    
                    int total = rs.getInt("totalRequests");
                    statistics.put("approvalRate", total > 0 ? (double) rs.getInt("approvedRequests") / total * 100 : 0);
                }
            }
            
        } catch (SQLException e) {
            System.err.println("Error getting monthly leave statistics: " + e.getMessage());
        }
    ...
    /**
     * Get leave requests by type and date range
     * @param leaveTypeId
     * @param startDate
     * @param endDate
     * @return 
     */
    public List<LeaveRequestModel> getLeaveRequestsByTypeAndDateRange(int leaveTypeId, Date startDate, Date endDate) {
        String query = "SELECT leaveRequestId, employeeId, leaveTypeId, leaveStart, leaveEnd, " +
                      "leaveReason, approvalStatus, dateCreated, dateApproved, supervisorNotes " +
                      "FROM leaverequest " +
                      "WHERE leaveTypeId = ? AND ((leaveStart BETWEEN ? AND ?) OR (leaveEnd BETWEEN ? AND ?)) " +
                      "ORDER BY leaveStart";
        ...
        try (Connection conn = getConnection();
            ...
            System.err.println("Error getting leave requests by type and date range: " + e.getMessage());
        }
    ...
    /**
     * Cancel leave request (only if pending or future approved)
     * @param leaveRequestId
     * @param employeeId
     * @param reason
     * @return 
     */
    public boolean cancelLeaveRequest(int leaveRequestId, int employeeId, String reason) {
        try (Connection conn = getConnection()) {
            ...
            String cancelNote = "Cancelled by employee. Reason: " + reason;
            logApprovalAction(conn, leaveRequestId, employeeId, "CANCELLED", cancelNote);
            
            // Delete the request
            return deleteLeaveRequest(leaveRequestId);
    ...
    /**
     * Get leave utilization report for department
     * @param department
     * @param year
     * @return 
     */
    public Map<String, Object> getDepartmentLeaveUtilization(String department, int year) {
        String query = "SELECT " +
                      "COUNT(lr.leaveRequestId) as totalRequests, " +
                      "SUM(CASE WHEN lr.approvalStatus = 'Approved' THEN DATEDIFF(lr.leaveEnd, lr.leaveStart) + 1 ELSE 0 END) as totalLeaveDays, " +
                      "COUNT(DISTINCT lr.employeeId) as employeesWithLeave, " +
                      ...
                      "WHERE p.department = ? AND YEAR(lr.leaveStart) = ?";
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    utilization.put("department", department);
                    utilization.put("year", year);
                    utilization.put("totalRequests", rs.getInt("totalRequests"));
                    utilization.put("totalLeaveDays", rs.getInt("totalLeaveDays"));
                    utilization.put("employeesWithLeave", rs.getInt("employeesWithLeave"));
                    utilization.put("avgLeaveDays", rs.getDouble("avgLeaveDays"));
                }
            }
