...
public class LeaveTypeDAO {
    ...
    /**
     * Validate leave type for employee request using Model business logic
     * @param leaveTypeId
     * @param employeeId
     * @param requestedDays
     ...
     * @return 
     */
    public boolean validateLeaveTypeForEmployee(Integer leaveTypeId, Integer employeeId, 
                                              int requestedDays, int alreadyUsedDays) {
        if (leaveTypeId == null || employeeId == null) {
            return false;
        }
        
        LeaveTypeModel leaveType = getLeaveTypeById(leaveTypeId);
        
        if (leaveType == null) {
            return false;
        }
        
        // Use the model's business logic for validation
        return leaveType.isWithinAnnualLimit(requestedDays, alreadyUsedDays);
    }
    
    /**
     * Validate leave request dates and limits
     * @param leaveTypeId
     ...
     * @return 
     */
    public boolean validateLeaveRequest(Integer leaveTypeId, LocalDate leaveStartDate, 
                                      LocalDate leaveEndDate, int alreadyUsedDays, 
        ...
        // Use the model's comprehensive validation
        return leaveType.isValidLeaveRequest(leaveStartDate, leaveEndDate, alreadyUsedDays, attendanceDates);
    }
    
    /**
     * Get leave validation message using Model
     * @param leaveTypeId
     * @param leaveStartDate
     * @param leaveEndDate
     * @param alreadyUsedDays
     * @return 
     */
    public String getLeaveValidationMessage(Integer leaveTypeId, LocalDate leaveStartDate, 
                                          LocalDate leaveEndDate, int alreadyUsedDays) {
        if (leaveTypeId == null) {
        ...
        return leaveType.getLeaveValidationMessage(leaveStartDate, leaveEndDate, alreadyUsedDays);
    }
    
    /**
     * Get leave type statistics for a specific year
     * @param year
     * @return 
     */
    public List<Map<String, Object>> getLeaveTypeStatistics(int year) {
        List<Map<String, Object>> statistics = new ArrayList<>();
        
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(SELECT_LEAVE_TYPE_STATISTICS)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    stats.put("leaveTypeId", rs.getInt("leaveTypeId"));
                    stats.put("leaveTypeName", rs.getString("leaveTypeName"));
                    stats.put("totalRequests", rs.getInt("totalRequests"));
                    stats.put("approvedRequests", rs.getInt("approvedRequests"));
                    stats.put("avgLeaveDays", rs.getDouble("avgLeaveDays"));
                    
                    int total = rs.getInt("totalRequests");
                    int approved = rs.getInt("approvedRequests");
            ...
            System.err.println("Error getting leave type statistics: " + e.getMessage());
        }
    ...
    /**
     * Get leave type utilization report
     * @param leaveTypeId
     * @param year
     * @return 
     */
    public Map<String, Object> getLeaveTypeUtilizationReport(Integer leaveTypeId, int year) {
        if (leaveTypeId == null) {
            return new HashMap<>();
        }
        ...
        String query = "SELECT " +
                      ...
                      "lt.maxDaysPerYear, " +
                      "COUNT(lr.leaveRequestId) as totalRequests, " +
                      "SUM(CASE WHEN lr.approvalStatus = 'Approved' THEN DATEDIFF(lr.leaveEnd, lr.leaveStart) + 1 ELSE 0 END) as totalDaysUsed, " +
                      "COUNT(DISTINCT lr.employeeId) as employeesUsed, " +
                      "AVG(CASE WHEN lr.approvalStatus = 'Approved' THEN DATEDIFF(lr.leaveEnd, lr.leaveStart) + 1 ELSE NULL END) as avgDaysPerRequest " +
                      "FROM leavetype lt " +
                      "LEFT JOIN leaverequest lr ON lt.leaveTypeId = lr.leaveTypeId AND YEAR(lr.leaveStart) = ? " +
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    report.put("leaveTypeId", leaveTypeId);
                    report.put("leaveTypeName", rs.getString("leaveTypeName"));
                    
                    // Handle null maxDaysPerYear
                    Integer maxDays = rs.getObject("maxDaysPerYear", Integer.class);
                    report.put("maxDaysPerYear", maxDays);
                    
                    report.put("totalRequests", rs.getInt("totalRequests"));
                    report.put("totalDaysUsed", rs.getInt("totalDaysUsed"));
                    report.put("employeesUsed", rs.getInt("employeesUsed"));
                    report.put("avgDaysPerRequest", rs.getDouble("avgDaysPerRequest"));
                    report.put("year", year);
                    
                    int totalUsed = rs.getInt("totalDaysUsed");
                    int employeesUsed = rs.getInt("employeesUsed");
                    
                    if (maxDays != null && maxDays > 0 && employeesUsed > 0) {
                        double utilizationRate = (double) totalUsed / (maxDays * employeesUsed) * 100;
            ...
            System.err.println("Error getting leave type utilization report: " + e.getMessage());
        }
    ...
    /**
     * Deactivate leave type (set maxDaysPerYear to 0)
     * @param leaveTypeId
     * @return 
     */
    public boolean deactivateLeaveType(Integer leaveTypeId) {
        ...
        String query = "UPDATE leavetype SET maxDaysPerYear = 0 WHERE leaveTypeId = ?";
     ...
     * @param leaveTypeId
     * @param maxDaysPerYear
     * @return 
     */
    public boolean reactivateLeaveType(Integer leaveTypeId, Integer maxDaysPerYear) {
        if (leaveTypeId == null || leaveTypeId <= 0 || maxDaysPerYear == null || maxDaysPerYear <= 0) {
            return false;
        }
        
        String query = "UPDATE leavetype SET maxDaysPerYear = ? WHERE leaveTypeId = ?";
        
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            
            stmt.setInt(1, maxDaysPerYear);
            stmt.setInt(2, leaveTypeId);
            return stmt.executeUpdate() > 0;
            
        } catch (SQLException e) {
            System.err.println("Error reactivating leave type: " + e.getMessage());
        }
    ...
    /**
     * Get leave types for dropdown/selection
     * @return 
     */
    ...
    /**
     * Calculate weekdays in leave period using Model logic
     * @param leaveStartDate
     * @param leaveEndDate
     * @return 
     */
    public int calculateLeaveDays(LocalDate leaveStartDate, LocalDate leaveEndDate) {
        return LeaveTypeModel.calculateWeekdaysInLeave(leaveStartDate, leaveEndDate);
    }
    
    /**
     * Check for leave-attendance conflicts using Model logic
     * @param leaveDate
     * @param attendanceDates
     * @return 
     */
    public boolean hasLeaveAttendanceConflict(LocalDate leaveDate, List<LocalDate> attendanceDates) {
        return LeaveTypeModel.hasLeaveAttendanceConflict(leaveDate, attendanceDates);
    }
