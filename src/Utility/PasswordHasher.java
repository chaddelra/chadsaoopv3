...
public class PasswordHasher {
    ...
    public static PasswordValidationResult validatePasswordStrength(String password) {
        ...
        if (containsPersonalInfoPattern(password)) {
            issues.add("Password may contain personal information");
            suggestions.add("Avoid using personal information like names or birthdates");
    ...
    /**
     * Validate password for employee registration
    ...
    public static PasswordValidationResult validateEmployeePassword(String password, 
        ...
        // Check if password contains employee's personal information
        if (password != null && firstName != null && lastName != null && email != null) {
            String lowerPassword = password.toLowerCase();
            
            // Check first name
            if (firstName.length() >= 3 && lowerPassword.contains(firstName.toLowerCase())) {
                additionalIssues.add("Password should not contain your first name");
                adjustedScore = Math.max(0, adjustedScore - 15);
            }
            
            // Check last name
            if (lastName.length() >= 3 && lowerPassword.contains(lastName.toLowerCase())) {
                additionalIssues.add("Password should not contain your last name");
                adjustedScore = Math.max(0, adjustedScore - 15);
            }
            ...
            if (email.contains("@")) {
                ...
                if (emailUser.length() >= 3 && lowerPassword.contains(emailUser)) {
                    additionalIssues.add("Password should not contain parts of your email address");
                    adjustedScore = Math.max(0, adjustedScore - 10);
                }
            }
            
            // Check for common ID patterns (employee IDs are often numeric)
            if (lowerPassword.matches(".*\\d{4,}.*")) {
                additionalSuggestions.add("Avoid using employee ID numbers in your password");
            }
        }
        
        // Additional payroll system specific checks
        if (password != null) {
            String lowerPassword = password.toLowerCase();
            
            // Check for payroll-related terms
            String[] payrollTerms = {"payroll", "salary", "motorph", "motor", "employee", "hr", "admin"};
            for (String term : payrollTerms) {
                if (lowerPassword.contains(term)) {
                    additionalIssues.add("Password should not contain company or payroll-related terms");
                    adjustedScore = Math.max(0, adjustedScore - 10);
                    break;
                }
            }
            
            // Check for Philippine-specific terms that might be easily guessable
            String[] localTerms = {"manila", "philippines", "pinoy", "pilipinas"};
            for (String term : localTerms) {
                if (lowerPassword.contains(term)) {
                    additionalSuggestions.add("Consider avoiding location-specific terms");
                    break;
                }
            }
        }
    ...
    private static boolean containsPersonalInfoPattern(String password) {
        ...
        if (lower.matches(".*\\b(name|birthday|birth|date|year|month|day|phone|email|address)\\b.*")) {
            return true;
        }
